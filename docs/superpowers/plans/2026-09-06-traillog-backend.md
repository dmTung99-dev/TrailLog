...
- **Monorepo path prefix:** this backend lives in `backend/` within the same repository as the TrailLog mobile app (which already occupies the repo root — `package.json`, `tsconfig.json`, `src/`, etc. at the root belong to the mobile app, not this plan). Every file path below (`package.json`, `src/main.ts`, `prisma/schema.prisma`, `test/health.e2e-spec.ts`, `.github/workflows/backend-ci.yml`, etc.) is written relative to `backend/` and must be created/modified at `backend/<path>`, not at the repo root. Every shell command below (`npm install`, `npx jest ...`, `docker compose up`, `git add <path>`, etc.) must run with `backend/` as the working directory — `cd backend` first, or use an equivalent `-C backend`/cwd mechanism — except `git commit`, which runs from the repo root as usual once files are staged with their `backend/`-prefixed paths (e.g. `git add backend/package.json backend/tsconfig.json ...`). The one exception is `.github/workflows/backend-ci.yml` (Task 6): GitHub Actions workflow files must live at the repo-root `.github/workflows/`, not `backend/.github/workflows/` — that task's CI steps should each run with `working-directory: backend` (or `cd backend &&` prefixes) inside the job.
